...
class Election {
    Process *p;  //pointer to dynamically allocate an array of process objects
    int *flag; //array that tracks the status of each process
    int num;
    ...
    void input() { 
        ...
        p = new Process[num];  //array of process objects of size num
        flag = new int[num];
    ...
    Process bully (Process coordinator) {
        for(int i=0;i<num;i++)  //make everyone's flag zero
            flag[i] = 0;

        int crashed = highest(p);  //make highest process' flag 1
        flag[crashed] = 1;

        for(int i=0;i<num;i++) {   //message sent to all the processes greater than i_coord
            if(p[i].id > coordinator.id) 
                cout<<"Election message sent from Process (coordinator) "<<coordinator.id<<" to Process "<<p[i].id<<endl;
            else flag[i] = 1;  //flag of processes smaller than i_coord crashed
        }
...
int main() {
    Election e;
    e.input();
    Process i_coord;  //initial coordinator
    Process f_coord;  //final coordinator
